...
共享单车项目，用户可以非常方便的外出骑行。独立完成整个 App 从无到有的开发、上架以及后续的升级维护工作。
在这个项目中，使用了 MVVM 架构，在后期更改需求的过程中，非常方便的就完成了需求的更换和迭代。

* 使用高德地图SDK 实现地图功能，在开发难度上低于百度地图SDK，后期新功能的实现相当迅速；
* 在布局方面使用了 Interface Builder 和 Masonry，根据标注图来做，能很完美的实现设计师所想展示的页面；
* 使用 BlowFish 并搭配 Base64 对传输数据进行加密和解密，使用用户的某条数据来作为 key，能很好的防止被抓包；
...
芳品商贸连锁店门店终端定货专用，独立开发，实现包括但不限于支付，购物车，报表等功能。
用户提出的新需求以及bug，处理一般不会超过一天。项目在提交之后，用户非常满意，后续依然由我们维护。
